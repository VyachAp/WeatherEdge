...
class TestCheckAndRecordDailyMaxAlert:
    ...
    async def test_pushes_when_projection_beats_forecast(self):
        ...
        # `alerted` is still computed even though commit 50a9a13 removed the
        # Telegram push — kept for downstream consumers (forecast_archive
        # backtests gate on it) and as a calibration label.
    ...
    # Removed: ``test_celsius_station_message_uses_celsius`` and
    # ``test_alert_displays_short_window_trend_when_present`` — both
    # asserted the Telegram message text (°F vs °C, displayed slope),
    # but commit 50a9a13 removed the actual Telegram push from
    # ``check_and_record_daily_max_alert``. The DB-row tests above and
    # below still cover the ``alerted`` decision flag.

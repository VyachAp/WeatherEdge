...
Settings: `LOCK_RULE_ENABLED`, `LOCK_RULE_{MIN,MAX}_PRICE`, `LOCK_MARGIN_F`, `LOCK_POSITION_PCT`, `FAST_LOCK_POLL_{ENABLED,INTERVAL_SECONDS}`. Range constants are module-level in `lock_rules.py`: `RANGE_LOCK_MIN_ROUTINES=4`, `RANGE_LOCK_MARGIN_MULTIPLIER=2.0`. **`RANGE_OVERSHOOT_LOCK_ENABLED` (Settings, default False, added 2026-05-22)** disables the `range_overshoot` NO branch: live data showed it lost -$57.61 / 56% win across many °C cities (Toronto/Buenos Aires/Shanghai/…) — our routine-METAR daily max reads hotter than Polymarket's resolver, so the "obviously locked" overshoot fires on a max the resolver never recorded. `range_undershoot` (+$8, 100%) and `range_in_window` are unaffected and read the flag live off `settings`. **CAVEAT (2026-05-30):** the -$57.61 / 56% figure is **pre-phantom-LOST-recovery** accounting. After running `admin reconcile-stuck --include-lost` the same branch shows **+$17 / 88% win / 25 trades all-time**. The disable may be a phantom-LOST artifact, *but* don't re-enable yet — the underlying °C resolver-divergence theory still has merit (see `docs/improvements.md`). Re-validate against post-recovery ground truth before flipping.
...
- `EXACTLY_MAX_LEAD_HOURS` (12.0, added 2026-05-22) — in the **probability path** (`binary_market_edge`), bracket-like markets (`exactly`/`range`/`bracket`) evaluated more than this many hours before the forecast **peak** are rejected with reason `bracket-like lead …h > Nh to peak`. **Lead is now measured to peak, not close (2026-05-23)** — for cities whose market close precedes the peak (e.g. Amsterdam: close 12:00 UTC, peak 14:00 UTC) the old time-to-close measure let pre-dawn bets through while the day's heating was still entirely ahead. Falls back to time-to-close when `hours_until_peak` is unknown. Live data: `exactly` prob-path trades lost -$126 in the 12-24h band but made +$57 in 0-12h. Threshold markets are exempt. The lock path is unaffected (observation-driven, naturally near peak)
- `BRACKET_LIKE_NO_DISABLED` (False, added 2026-05-30; **`.env` overrides to True**) — **master kill switch** for bracket-like NO (`exactly`/`range`/`bracket` BUY_NO) in the **probability path**. When True, `binary_market_edge` rejects any otherwise-passing bracket-like NO edge with reason `"bracket-like NO disabled (sigma recalibration pending)"`. Fires after Layers 1-3 under the `reason is None` guard, so a more specific reject reason wins in `evaluation_logs` — this label only marks the otherwise-passing residual. Default False = no behavior change. **Motivation (mixed pre- and post-recovery figures, see CAVEAT below):** the pre-recovery numbers were `exactly`-NO all-time -$260 / -7.4% ROI / 383 trades and 14d -$346 / -14.3%; **post-recovery the same all-time figure is +$169 / +5.4% ROI** but the recent 7d window (post-recovery, real fills only) still shows `exactly`-NO at -$139 / -8.2% / 188 trades, so the *directional* "recent class is -EV" conclusion holds. Even at the tightened single-bucket guards (margin=2.5°F, cap=0.85) the live `evals-report --operator bracket-like` tuner shows surviving NO evals at **-0.023 EV/$1**, and no `(margin, cap)` pair produces a +EV survivor set short of "block everything". The structural fix (lead-time-aware σ floor) is backlogged in `docs/improvements.md` because it would also retune the working threshold path — until it ships, this flag is the operational kill. **Scope:** probability path only. Lock path (`evaluate_lock` → `_evaluate_range_lock`) does NOT call `binary_market_edge`, so `range_in_window` YES, `range_undershoot` YES, and `exactly` lock YES branches keep firing. Threshold ops (`above`/`at_least`/`below`/`at_most`) unaffected. `exactly` YES (probability path) unaffected. **Reactivation criterion:** flip back to False once the σ-recalibration entry lands and a clean re-run of `evals-report --operator bracket-like` shows the tuner's "baseline" (all-passing) row at +EV
- `SINGLE_BUCKET_NO_BAND_MARGIN_F` (**2.5**, added 2026-05-23 @ 1.0, tightened to 2.5 on 2026-05-26) + `SINGLE_BUCKET_MAX_NO_PROB` (**0.85**, added 2026-05-23 @ 0.92, tightened to 0.85 on 2026-05-26) — two of the **three single-bucket NO guards** in `binary_market_edge` (the third reuses `EXACTLY_MAX_LEAD_HOURS` above). The 2026-05-26 retune (margin 1.0→2.5°F ≈ 1.4°C, cap 0.92→0.85) hardens the `exactly` NO class — the #1 real-money loss source (61% win vs ~78% breakeven on fills). Margin 2.5 hedges the ±~1°C divergence between our routine-METAR daily max and Polymarket's resolver; cap 0.85 + `MIN_EDGE=0.10` restricts NO to prices ≤ ~0.75. Validated with `evals-report --operator bracket-like` (the new single-bucket-NO-guard-tuning section grids margin×cap and scores blocked-vs-survivor against the corrected-day daily max). **Much of the historical `exactly` bleed was actually the date-misalignment (day-early Americas bets) fixed the same day — see the data-day gotcha; scored on the corrected day the surviving NO bets are ~+EV.** Defense against the incoherent "NO on every adjacent single-°C bucket" pattern (-$163.84 / 179 trades in the `model_prob ≥ 0.999` band, many °C cities). **Landing-band guard:** refuse a NO bet on a single-bucket (`exactly`/`range`/`bracket`) window overlapping `[current_max_f − margin, max(forecast_peak_f, current_max_f) + margin]` (the band collapses toward `current_max_f` once past peak) — never bet against a bucket the day could still plausibly land in. **Overconfidence cap:** floor `our_prob_yes` so NO confidence can't exceed `SINGLE_BUCKET_MAX_NO_PROB`. Both NO-side only; both no-ops for threshold ops and when forecast context is `None` (legacy callers)
- `THRESHOLD_MIN_PROBABILITY` / `THRESHOLD_MIN_EDGE` (both `None`, added 2026-05-23) — per-operator-class loosening of `MIN_PROBABILITY` / `MIN_EDGE`. Applied by `binary_market_edge` to **threshold ops only**; bracket-like ops keep the strict globals. `None` = use global (no-op). Set via `.env` *after* validating with `evals-report --operator threshold` (the recoverable-band section recomputes `_check_filters` over candidate values and scores would-have-won vs the routine-METAR daily max). 7-day prod telemetry: the `prob∈[0.78,0.85)` threshold band wins ~91% at +0.57 EV/$1; `prob<0.75` degrades. **Pair with a sizing review:** `decisions-report` shows `stake_below_min` ($5 floor at small bankroll) is the dominant downstream throttle, so filter loosening recovers eval-volume but a chunk stalls before becoming a trade. **CAVEAT (2026-05-30):** the live `.env` has had `THRESHOLD_MIN_PROBABILITY=0.78` for ≥7 days and produces **0 fills with `signals.model_prob ∈ [0.78, 0.85)`** — `apply_calibration` squashes the band toward the >0.85 tail (training set is 0.95+-dominated) AND Kelly × `KELLY_PROB_CAP` × the lower edges at a ~$300 bankroll lands sub-MIN_STAKE_USD. The `evals-report` "recoverable band" measures *passing-eval volume*, not fills. Further loosening (→0.75) is a no-op until the calibrator is rebuilt or `NEAR_PEAK_FLOOR_UP_MIN_PROB` is lowered to match. See memory `project_threshold_loosening_no_op_2026-05-30`.
...
- **Data day = `end_date`'s UTC date = the question-text day (`resolve_target_local_day`, fixed 2026-05-26).** Polymarket daily-temp markets close at `end_date` = the title day at **noon UTC** (verified: 29,940/30,060 markets at 12:00 UTC; `end_date::date (UTC)` == `parsed_target_date` exactly). That UTC date IS the station-local data day for BOTH hemispheres (cross-checked resolved trade WON/LOST vs actual METAR daily maxes: São Paulo "May 25" → SP May 25 = 22°C; Seoul "May 25" → Seoul May 25 = 27°C). **The prior code subtracted 12h from `end_date` then localized — correct for +UTC cities by luck (midnight-UTC anchor stays on the title day after a +9..+13 offset) but returned title-day−1 for −UTC cities** (the anchor rolls back to the previous local evening). That made `should_skip_future_day` fail to skip the *next-day* Americas market, so the bot bet e.g. São Paulo's "May 27" market at 4 AM on May 26 using May 26's weather state — and `_market_daily_max` anchored the lock path to the wrong day too. Both are fixed by the single `resolve_target_local_day` change. **Consequence: Americas trade volume drops** — a −UTC city's noon-UTC "today" market expires by local morning (before its peak) and the next-day market is now correctly skipped; those markets weren't profitable and the day-early behavior was unsound. The legacy UTC-date fallback in `should_skip_future_day` (no `station_icao`) is unchanged.
...
- **`exactly` markets are lead-gated in the probability path (2026-05-22; lead measured to peak as of 2026-05-23).** `binary_market_edge` rejects bracket-like edges (`exactly`/`range`/`bracket`) evaluated more than `EXACTLY_MAX_LEAD_HOURS` (12) before the forecast **peak** (`hours_until_peak`, falling back to time-to-close when unknown) — they were the bot's worst trade class (-$107.53 all-time), losing -$126 in the 12-24h lead band while making +$57 in 0-12h. The gate fires *after* `_check_filters` under a `reason is None` guard, so an edge that already failed keeps its original reason; the `bracket-like lead …` reason only marks edges that would otherwise have passed. Threshold ops are exempt.
- **`place_order` does a wallet pre-flight before submitting (2026-05-30).** Right after the daily-spend-cap check, `place_order` calls `get_wallet_usdc_balance(force_refresh=True)`; if the live CLOB-visible spendable is less than `trade.stake_usd`, the order is short-circuited with `trade.exchange_status = "insufficient_balance"` and `return False`. Callers (`scheduler/__init__.py` probability path + `lock_rule_executor.try_lock_rule_trade`) route this status to the new `OUTCOME_INSUFFICIENT_BALANCE` `decision_log` outcome (otherwise `OUTCOME_ORDER_FAILED`) — separates "intentional hold-back" from "tried and lost" in dashboards. **Why:** the local exposure cap is fed by the DB view of OPEN/PENDING trades, which lags the on-chain reality (phantom-OPEN rows). Burst placements within a tick raced the CLOB's per-order spendable check, producing 140 `exception:PolyApiException` rows in 14d (May 2026). Skip path is no-op for dry-run (the dry-run guard runs first) and for `get_wallet_usdc_balance()→None` (transient CLOB outage / no private key — preserves prior behavior). One extra CLOB call per placement, well inside the 10 rps limit. Memory: `project_polyapi_balance_race_2026-05-30`.
- **Bracket-like NO master switch is on in the live `.env` (2026-05-30).** `BRACKET_LIKE_NO_DISABLED=True` rejects every `exactly`/`range`/`bracket` BUY_NO edge in the probability path with reason `"bracket-like NO disabled (sigma recalibration pending)"`. The single-bucket guards (margin/cap/lead) still run first, so a more specific reject reason still wins in `evaluation_logs` — the master-switch label only marks the otherwise-passing residual. **Flip back to False** only after the lead-time-aware σ-floor work (backlog in `docs/improvements.md`) ships and a fresh `evals-report --operator bracket-like` shows the tuner's baseline (all-passing) row at +EV.
- **Single-bucket NO guards (2026-05-23).** Three layered guards in `binary_market_edge` kill the incoherent "NO on every adjacent single-°C bucket" pattern (e.g. Amsterdam 2026-05-23 bet NO on 27/28/29°C while the blended forecast was pinned at 30°C with a σ collapsed to ~1.1°C because 4 ensemble models agreed). Root cause: each single-°C bucket is evaluated as an **independent** binary, so a tight Gaussian centered on an over-forecast peak makes every neighbouring bucket look near-impossible → NO passes on all of them. The `model_prob ≥ 0.999` NO band cost -$163.84 / 179 trades across many °C cities. Guards (all NO-side, all `op in (exactly,range,bracket)` only, all no-op when forecast context is `None`): (1) peak-relative lead gate (above); (2) **landing-band** — refuse NO on a window overlapping `[current_max_f − margin, max(forecast_peak_f, current_max_f) + margin]` (`SINGLE_BUCKET_NO_BAND_MARGIN_F`), collapsing to `current_max_f` once past peak; (3) **overconfidence cap** — floor `our_prob_yes` so NO ≤ `SINGLE_BUCKET_MAX_NO_PROB` (0.92, **tightened to 0.85 on 2026-05-26**; margin also 1.0→**2.5**°F — see the Settings section and the data-day gotcha). `state.forecast_peak_f`/`current_max_f`/`hours_until_peak` are threaded into `binary_market_edge` from the one call site in `job_unified_pipeline`. **σ recalibration (lead-time-aware floor so a tight ensemble can't collapse σ far from peak) was deliberately deferred** — see `docs/improvements.md`.
- **`range_overshoot` NO lock is disabled by default (2026-05-22).** `RANGE_OVERSHOOT_LOCK_ENABLED=False` — the branch lost -$57.61 / 56% win across many °C cities (resolver/observation divergence, not margins; 2× margin was already applied). `range_undershoot`/`range_in_window` still fire. Re-enabling requires fixing the °C resolver divergence first (see `docs/improvements.md`).

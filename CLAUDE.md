...
- `EXACTLY_MAX_LEAD_HOURS` (12.0, added 2026-05-22) — in the **probability path** (`binary_market_edge`), bracket-like markets (`exactly`/`range`/`bracket`) evaluated more than this many hours before close are rejected with reason `bracket-like lead …h > Nh max`. Live data: `exactly` prob-path trades lost -$126 in the 12-24h lead band but made +$57 in 0-12h (far from peak the Gaussian collapses P(a single ~2°F bucket) → ~0, inventing NO edge). Threshold markets are exempt. The lock path is unaffected (observation-driven, naturally near close)
...
- **`exactly` markets are lead-gated in the probability path (2026-05-22).** `binary_market_edge` rejects bracket-like edges (`exactly`/`range`/`bracket`) evaluated more than `EXACTLY_MAX_LEAD_HOURS` (12) before close — they were the bot's worst trade class (-$107.53 all-time), losing -$126 in the 12-24h lead band while making +$57 in 0-12h. The gate fires *after* `_check_filters` under a `reason is None` guard, so an edge that already failed keeps its original reason; the `bracket-like lead …` reason only marks edges that would otherwise have passed. Threshold ops are exempt.

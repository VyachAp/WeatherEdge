...
Settings: `LOCK_RULE_ENABLED`, `LOCK_RULE_{MIN,MAX}_PRICE`, `LOCK_MARGIN_F`, `LOCK_POSITION_PCT`, `FAST_LOCK_POLL_{ENABLED,INTERVAL_SECONDS}`. Range constants are module-level in `lock_rules.py`: `RANGE_LOCK_MIN_ROUTINES=4`, `RANGE_LOCK_MARGIN_MULTIPLIER=2.0`. **`RANGE_OVERSHOOT_LOCK_ENABLED` (Settings, default False)** disables the `range_overshoot` NO branch because our routine-METAR daily max reads hotter than Polymarket's resolver in °C cities, so the "obviously locked" overshoot fires on a max the resolver never recorded. `range_undershoot` and `range_in_window` are unaffected (read the flag live off `settings`). The original loss figures were phantom-LOST-inflated and net out near flat post-recovery, but the °C resolver-divergence theory still holds — **don't re-enable until that divergence is fixed** (`docs/improvements.md`).
...
For A/B logging, every exceedance line carries `projected=… (live v2), legacy_projected=… (v1)`. Promote/revert via `PROJECTION_RESIDUAL_SLOPE_ENABLED` — code default is `True`, but **the live `.env` currently sets it `False`** (v2 overshoots the warming concavity at high-volume stations like KLAX/KJFK/RKSI/LFPG), so **v1 is the active projector pending a v3 redesign**.
...
|------|---------|-------------|
| `src/scheduler/__init__.py` | APScheduler jobs + setup. Imports many helpers from submodules (`execution.binary_market`, `persistence.cache_rollover`, `persistence.dedup`, `signals.edge_calculator`, `signals.evaluation_log`, `risk.cluster_cap`, etc.) under `_X` aliases — historical from when those helpers were defined here as private. The aliases are NOT the public API; only `run_scheduler`/`configure_logging`/`backfill_markets`/`start_health_server` are. Tests reach a few `_X` aliases (`_binary_market_edge`, `_has_active_trade`, `_log_evaluation`, `_minimal_state_for_easy_lock`, `_should_skip_future_day`, `_upsert_signal`) for unit coverage; new code should import from the canonical home modules instead. Future internal split into `jobs.py` + `setup.py` is deferred (see `docs/improvements.md`). | **Public:** `run_scheduler`, `configure_logging`, `backfill_markets`, `start_health_server`. **Scheduler-local (tested via underscore):** `_load_stuck_alert_cooldown`, `_persist_stuck_alert_cooldown`, `_fast_poll_projection_check`. Jobs: see Scheduler Jobs table. |
...
| `src/signals/market_resolution.py` | Measurement layer M3 — pure resolved-max bound + resolver-divergence logic, a best-effort `MarketResolution` upsert (from `resolve_trades`), and `backfill_routine_max` (fills `routine_metar_max_f` + divergence per station-day from `job_daily_settlement`). De-circularises filter tuning vs the routine-METAR proxy. | `implied_max_bounds`, `divergence_f`, `record_market_resolution`, `backfill_routine_max` |
...
| `src/db/models.py` | SQLAlchemy ORM | `Market`, `Signal`, `Trade`, `EvaluationLog`, `DecisionLog`, `StationBias`, `MetarObservation`, `ForecastExceedanceAlert`, `ForecastArchive`, `ConfigEpoch`, `ExposureSnapshot`, `MarketResolution`, etc. |
...
| `src/cli.py` | CLI entry points | `run`, `scan`, `backfill`, `status`, `paper-trade`, `evals-report` (`--operator threshold\|bracket-like`; operator-split reject reasons + threshold recoverable-band win-rate/EV + **single-bucket NO guard tuning** (grids `SINGLE_BUCKET_NO_BAND_MARGIN_F` × `SINGLE_BUCKET_MAX_NO_PROB`, scores blocked-vs-survivor) — all vs routine-METAR daily max), `decisions-report` (post-filter funnel by `outcome` + operator class + sizing-stall `size_reason` + **floored-fill outcomes** — `trade_filled` joined to settled `Trade`, split by `metadata.floored_up`, Won%/break-even/EV-per-$1: the post-flip gate for `NEAR_PEAK_FLOOR_UP_ENABLED`), `exposure-report` (M4 per-tick headroom/capital gate), `resolution-report` (M3 per-station resolver divergence), `shadow-report` (M1 counterfactual telemetry), `epochs` (M2 config-epoch list + flag diff; `evals`/`decisions-report` take `--since-epoch`), `backtest-v2`, `migrate`, `approve`, `test-trade`, `bet {place,info,search,find,cancel,orders,portfolio,redeem}`, `admin {reset-drawdown-peak,reconcile-stuck [--include-lost]}` |
...
- **`projected_max_f` in `forecast_exceedance_alerts` is whichever path is live (code default v2, but the live `.env` sets `PROJECTION_RESIDUAL_SLOPE_ENABLED=False` → v1 is currently active).** The other version's value is only in JSON logs (`legacy_projected=`), not the DB.
...
- **`range_overshoot` NO lock is disabled by default** (`RANGE_OVERSHOOT_LOCK_ENABLED=False`) — °C resolver/observation divergence. `range_undershoot`/`range_in_window` still fire. See the lock-rule Settings entry; re-enabling requires fixing the divergence first.
...
- **`MIN_EDGE` is harmonised on `settings.MIN_EDGE` (default 0.05).** `_check_filters` reads `settings.MIN_EDGE` directly (the former module-level `edge_calculator.MIN_EDGE` re-export was removed). **The current `.env` has `MIN_EDGE=0.10`** — that env value wins via override; review before assuming default applies.

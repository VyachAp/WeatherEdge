...
| `src/signals/market_resolution.py` | Measurement layer M3 — pure resolved-max bound + resolver-divergence logic plus a best-effort `MarketResolution` upsert. De-circularises filter tuning vs the routine-METAR proxy. | `implied_max_bounds`, `divergence_f`, `record_market_resolution` |
